...
def main():
    ...
    assert max(origin_errors)<1e-6 and min(heights)>=-.060001 and max(heights)<=.060001
    path=Path('artifacts/infantry_audit/rugged_geometry.json');path.parent.mkdir(parents=True,exist_ok=True)
