...
def agent(iterations=900):
    cfg=rugged_agent(iterations);cfg.algorithm.learning_rate=8e-5;cfg.algorithm.entropy_coef=.003
    return cfg


def hashes():
    result=rugged_hashes()
    for name in ['mjlab_motion_task.py','motion_control.py','mjlab_motion_train.py','mjlab_motion_play.py','mjlab_motion_audit.py']:
...
def main():
    ...
    ap.add_argument('--envs',type=int,default=1024);ap.add_argument('--iterations',type=int,default=900)
    ...
    cfg=task.make_cfg(args.envs);cfg.seed=46;runner_cfg=agent(args.iterations)
    info=dict(task='v5_motion',model_variant='fix',model_sha256=meta['model_sha256'],observation_dim=56,action_dim=6,
        ...
        transfer='Full actor/critic and Adam continuation; LR 8e-5; std floor .06; normalization count 100000, moments retained')
    ...
    try:
        ...
        runner.alg.learning_rate=8e-5
        for group in runner.alg.optimizer.param_groups:group['lr']=8e-5
        ...
        with torch.no_grad():
            ...
            if dist.std_type=='scalar':dist.std_param.clamp_(min=.06)
            else:dist.log_std_param.clamp_(min=float(torch.log(torch.tensor(.06))))
        ...
        assert task.metric(env,'ground_hit_fraction').min()==1.
