...
def agent(iterations=400):
    cfg=rugged_agent(iterations);cfg.algorithm.learning_rate=5e-5;cfg.algorithm.entropy_coef=.002;cfg.save_interval=50
    return cfg


def hashes():
    result=rugged_hashes()
    for name in ['mjlab_motion_task.py','motion_control.py','mjlab_motion_train.py','mjlab_motion_play.py','mjlab_motion_audit.py','mjlab_jumpcarry_task.py','mjlab_jumpcarry_train.py','mjlab_jumpcarry_play.py','mjlab_jumpcarry_audit.py']:
...
def main():
    ...
    info=dict(task='v5_jumpcarry',model_variant='fix',model_sha256=meta['model_sha256'],observation_dim=56,action_dim=6,
        ...
        transfer='Full actor/critic and Adam continuation; LR 5e-5; std floor .08; normalization count 100000, moments retained')
