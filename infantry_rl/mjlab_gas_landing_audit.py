...
from infantry_rl import mjlab_gas_landing_task as task
...
def cases():
    wanted=['flat_drive_0.0','flat_drive_1.3','flat_high_0.0','flat_jump_0.0','flat_jump_0.5','flat_jump_1.0','flat_jump_1.5','flat_brake_2']
    return [dict(row,name=row['name']+'_gas'+str(force),force=force) for row in original_cases() if row['name'] in wanted for force in [0,100,200,300]]
...
def main():
    ...
    cfg=task.make_cfg(n,play=True,seed=1043,stage='tuck');cfg.seed=2046;cfg.episode_length_s=10.
    ...
    try:
        ...
        env._gas_force=values('force').float()
        ...
        min_reach=torch.full((n,),float('inf'),device=env.device);body_start=torch.zeros(n,device=env.device);body_peak=body_start.clone()
        ...
        motor_peak=count.clone();motor_sat=count.clone();load_samples=count.clone()
        with torch.no_grad():
            for step in range(450):
                if step==100:height[high]=.28
                if step==150:
                    body_start[:]=robot.data.root_link_pos_w[:,2]
                ...
                # XML appends the two tendon motors after the six drive motors.
                if step==10:assert torch.allclose(d.actuator_force[:,-2:],env._gas_force[:,None].expand(-1,2),atol=1e-3)
                loads=d.actuator_force[:,:6].abs()
                motor_peak=torch.maximum(motor_peak,torch.where(alive,loads.amax(1),0.))
                motor_sat+=alive*(loads[:,:4]>=29.5).any(1);load_samples+=alive
                ...
                if 150<=step<350:
                    ...
                    wheel_ids=robot.find_bodies(('l_wheel_Link','r_wheel_Link'),preserve_order=True)[0]
                    reach=d.root_link_pos_w[:,2]-d.body_link_pos_w[:,wheel_ids,2].amin(1)
                    min_reach=torch.minimum(min_reach,torch.where(air,reach,min_reach))
                    body_peak=torch.maximum(body_peak,torch.where(alive&jump,d.root_link_pos_w[:,2]-body_start,0.))
        ...
        for k,row in enumerate(rows):
            ...
            report['cases'][row['name']]=dict(force_per_side_N=row['force'],mode=row['mode'],speed=row['speed'],terrain=row['typ'],
                motor_peak_Nm=float(motor_peak[sl].max()),leg_saturation_fraction=float((motor_sat[sl]/load_samples[sl].clamp_min(1)).mean()),
                ...
                min_body_to_wheel_m=float(torch.where(torch.isfinite(min_reach[sl]),min_reach[sl],0.).mean()),body_rise_m=float(body_peak[sl].mean()),
        ...
        out=args.out or args.run/(checkpoint.stem+'_gas_audit.json');out.parent.mkdir(parents=True,exist_ok=True)
