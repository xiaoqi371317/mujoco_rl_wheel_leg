...
def main():
    ...
    if meta['task'] == 'cmd_vel':
        ...
        factory = task.make_cfg
    cases = [('stop', 0., 0.), ('forward', .3, 0.), ('reverse', -.2, 0.),
             ('left_arc', .3, .6), ('right_arc', .3, -.6),
             ('left_spin', 0., 1.2), ('right_spin', 0., -1.2), ('spin_then_stop', 0., 1.2)]
    repeats = 4
    ...
    desired = torch.tensor([[x, 0., z] for _, x, z in cases], device='cuda:0').repeat_interleave(repeats, 0)
    # Replace only command sampling, not physics/reset/reward or the policy.
    command.compute = lambda dt: command.vel_command_b.copy_(desired)
    alive = torch.ones(env.num_envs, dtype=torch.bool, device='cuda:0')
    sums = torch.zeros(env.num_envs, 10, device='cuda:0')
    ...
    try:
        ...
        with torch.inference_mode():
            for step in range(500):
                ...
                if step < 350:
                    active[-repeats:] = False
                sums += active[:, None] * torch.stack((vx, wz, ex, ez, torch.rad2deg(tilt), good,
                    d.root_link_lin_vel_b[:, 2].square(), joint_acceleration, action_smooth, joint_limits), 1)
                counts += active
        ...
        for i, (name, x, z) in enumerate(cases):
            group = slice(i*repeats, (i+1)*repeats)
            valid = counts[group] > 0
            mean = values[group][valid].mean(0).tolist() if valid.any() else [None]*10
            metric_names = ['vx', 'wz', 'vx_mae', 'yaw_mae', 'tilt_deg', 'tracking_fraction',
                'vertical_velocity_sq', 'joint_acceleration_norm_sq', 'action_smooth', 'joint_limit_violation']
            ...
            report['cases'][name]['passed'] = bool(alive[group].all() and (counts[group] > 0).all() and
                (values[group, 2] < .1).all() and (values[group, 3] < .2).all() and (values[group, 5] > .8).all())
