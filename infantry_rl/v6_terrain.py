"""V6 rugged terrain: 8 cm collision grid; retain amplitudes and world-space wavelengths."""
...
class V6TerrainCfg(SubTerrainCfg):
    kind: str='noise'
    def function(self,difficulty,spec,rng):
        ...
        nx,ny=round(sx/.08)+1,round(sy/.08)+1
        ...
        field=spec.add_hfield(name='v6_rugged_'+key,size=(sx/2,sy/2,zmax-zmin,.03),
    ...
    return TerrainEntityCfg(terrain_type='generator',max_init_terrain_level=0,textures=(),materials=(),
        terrain_generator=TerrainGeneratorCfg(seed=seed,curriculum=True,size=(28.,6.),num_rows=3,num_cols=3,
            color_scheme='none',sub_terrains={
                'flat':V6TerrainCfg(kind='flat',proportion=.30),
                'bumps_pits':V6TerrainCfg(kind='noise',proportion=.50),
                'ripples':V6TerrainCfg(kind='wave',proportion=.20)}))
