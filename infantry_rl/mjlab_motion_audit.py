...
def main():
    ...
    try:
        ...
        for k,row in enumerate(rows):
            ...
            if row['mode']=='jump':
                passed&=started[sl]&(peak[sl]>.02)&(airtime[sl]>=.06)&(upward[sl]>.5)
            ...
            report['cases'][row['name']]=dict(mode=row['mode'],speed=row['speed'],terrain=row['typ'],
                passed=bool(passed.all()),success_fraction=float(passed.float().mean()),
        ...
        out=args.out or args.run/(checkpoint.stem+'_motion_audit.json');out.parent.mkdir(parents=True,exist_ok=True)
