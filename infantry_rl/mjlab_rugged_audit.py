...
def main():
    ...
    try:
        ...
        out=args.out or args.run/(checkpoint.stem+'_rugged_audit.json');out.parent.mkdir(parents=True,exist_ok=True)
        out.write_text(json.dumps(report,indent=2));print('RUGGED_AUDIT',json.dumps(report),flush=True)
    finally:vec.close()
