"""Small smooth bumps/pits with a level spawn pad and explicit metric bounds."""
...
class GentleTerrainCfg(SubTerrainCfg):
    kind: str='noise'
    def function(self,difficulty,spec,rng):
        ...
        amplitude=.005+.010*difficulty
        nx,ny=round(sx/.05)+1,round(sy/.05)+1
        x=np.linspace(-sx/2,sx/2,nx);y=np.linspace(-sy/2,sy/2,ny)
        xx,yy=np.meshgrid(x,y)
        if self.kind=='noise':
            coarse=rng.uniform(-1,1,(round(sy/.25)+1,round(sx/.25)+1))
            ...
            z=(np.sin(xx*2*np.pi/.65+phase[0])*np.cos(yy*2*np.pi/.8+phase[1])
                +.3*np.sin(xx*2*np.pi/1.1+yy*3+phase[2]))/1.3
        ...
        field=spec.add_hfield(name='gentle_'+key,size=(sx/2,sy/2,zmax-zmin,.03),
    ...
    return TerrainEntityCfg(terrain_type='generator',max_init_terrain_level=0,textures=(),materials=(),
        terrain_generator=TerrainGeneratorCfg(seed=seed,curriculum=True,size=(28.,6.),num_rows=3,num_cols=3,
            color_scheme='none',sub_terrains={
                'flat':GentleTerrainCfg(kind='flat',proportion=.30),
                'bumps_pits':GentleTerrainCfg(kind='noise',proportion=.50),
                'ripples':GentleTerrainCfg(kind='wave',proportion=.20)}))
