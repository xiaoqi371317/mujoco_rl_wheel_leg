...
from infantry_rl import mjlab_v6_jump_task as task
...
def main():
    ...
    try:
        ...
        stance_sum=torch.zeros(n,2,device=env.device);stance_count=torch.zeros(n,device=env.device)
        ...
        with torch.no_grad():
            for step in range(450):
                ...
                pair,dx,_=task.stance_errors(env)
                stand=alive&task.stance_mask(env)&(step>100)
                stance_sum+=torch.stack((pair.abs().mean(1),dx.abs()),1)*stand[:,None];stance_count+=stand
        ...
        for k,row in enumerate(rows):
            ...
            carry_passed=passed.clone()
            if row['mode']=='jump':passed&=peak[sl]>=.10
            if row['typ']==0 and row['speed']==0 and row['yaw']==0 and row['mode'] in ('drive','high','rise'):
                passed&=(stance_count[sl]>50)&(stance_sum[sl,0]/stance_count[sl].clamp_min(1)<.05)&(stance_sum[sl,1]/stance_count[sl].clamp_min(1)<.015)
            report['cases'][row['name']]=dict(mode=row['mode'],speed=row['speed'],terrain=row['typ'],
                stance_pair_rad=float((stance_sum[sl,0]/stance_count[sl].clamp_min(1)).mean()),stance_wheel_dx_m=float((stance_sum[sl,1]/stance_count[sl].clamp_min(1)).mean()),stance_samples=float(stance_count[sl].sum()),
                passed=bool(passed.all()),carry_criteria_passed=bool(carry_passed.all()),original_criteria_passed=bool(original_passed.all()),
        ...
        out=args.out or args.run/(checkpoint.stem+'_v6_jump_audit.json');out.parent.mkdir(parents=True,exist_ok=True)
