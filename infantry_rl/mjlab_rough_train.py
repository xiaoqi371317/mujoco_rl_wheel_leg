"""Continue a V5 checkpoint on gentle rough ground, retaining Adam state."""
import argparse
...
def hashes():
    h=v5_hashes()
    for name in ['rough_terrain.py','mjlab_rough_task.py','mjlab_rough_train.py','mjlab_rough_audit.py','mjlab_rough_play.py']:
...
def main():
    ...
    ap.add_argument('--envs',type=int,default=1024);ap.add_argument('--iterations',type=int,default=800)
    ...
    info=dict(task='v5_rough',model_variant='fix',model_sha256=meta['model_sha256'],observation_dim=56,action_dim=6,
    ...
    try:
        ...
        env._rough_start_step=saved['infos']['env_state']['common_step_counter']-(24*500 if args.smoke else 0)
        ...
        print('ROUGH_RESUMED',args.resume or args.source,'iteration',initial,'Adam retained','terrain_hits',hits,flush=True)
