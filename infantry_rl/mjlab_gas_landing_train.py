...
def agent(iterations=400):
    cfg=rugged_agent(iterations);cfg.algorithm.learning_rate=2e-5;cfg.algorithm.entropy_coef=.001;cfg.save_interval=50
    return cfg


def hashes():
    result=rugged_hashes()
    for name in ['mjlab_gas_landing_task.py','mjlab_gas_landing_train.py','mjlab_gas_landing_audit.py','mjlab_gas_landing_stance_audit.py']:
...
def main():
    ...
    ap.add_argument('--stage',choices=['stance','tuck'],default='tuck')
    ...
    info=dict(task='gas_landing_continuation',stage=args.stage,model_variant='fix_with_gas_trial',model_sha256=meta['model_sha256'],observation_dim=56,action_dim=6,
        ...
        transfer='Full actor/critic and Adam continuation; LR 2e-5; std floor .08; normalization count 100000, moments retained')
    physics=Path(__file__).parent/'robot/xmls/training_scene_fix_gas_trial.xml'
    info.update(physics_model_sha256=hashlib.sha256(physics.read_bytes()).hexdigest(),physics_model_file=physics.name,gas_force_range_N=[200,300],gas_ramp_iterations=0,hardware_force_calibrated=False)
    assert meta.get('physics_model_sha256')==info['physics_model_sha256'], 'Source physics mismatch'
    assert teacher_meta.get('physics_model_sha256')==info['physics_model_sha256']
    ...
    try:
        ...
        runner.alg.learning_rate=2e-5
        for group in runner.alg.optimizer.param_groups:group['lr']=2e-5
